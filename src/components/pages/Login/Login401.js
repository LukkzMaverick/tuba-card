...
function Login401() {
  ...
  const [mostrarAlertError, setMostrarAlertError] = useState(true)
  const [mensagensErro, setMensagensErro] = useState(['Token expirado, por favor faça o login novamente!'])
  ...
  return (
    ...
    <div className='container'>
        ...
        {mostrarAlertError ? <Alert closeText="Fechar" onClose={() => setMostrarAlertError(false)} className={classes.alertError} severity={alertSeverity}>
          <ul>
          {mensagensErro}
          </ul>
        </Alert> : ''}
